...
import {
  getBsiSampahMonitoring,
  type BsiSampahMonitoring,
} from '../../api/monitoring';
import type {AdminHomeStackParamList} from '../../navigation/stacks/AdminHomeStackNavigator';
...
function buildYearSeries(
  beratMap: BsiSampahMonitoring['beratPerKategoriByMonthYear'] | undefined,
  emisiMap:
    | BsiSampahMonitoring['emisiKarbonPerKategoriByMonthYear']
    | undefined,
  year: number,
): MonthPoint[] {
...
export function AdminMonitoringScreen(): React.JSX.Element {
  ...
  const [sampah, setSampah] = React.useState<BsiSampahMonitoring | null>(null);
  ...
  const load = React.useCallback(async () => {
    ...
    const res = await getBsiSampahMonitoring(user.token);
    ...
    <Screen
      ...
      <View style={styles.topActions}>
        <AppButton
          ...
          variant="secondary"
          onPress={() => navigation.navigate('AdminNasabahLeaderboard')}
        />
      </View>
